...
replaceExact(
  ...
  "if(s)s.textContent='UNIFIED S³ · QUEST-SAFE WEBXR · IMMERSIVE LAB · v4.6.1';"
);
replaceExact(
...
replaceExact(
  'insert Quest XR startup recovery',
  '/* ------------------------------------------------------------- session */\nasync function enterImmersive450(){',
  `${xrRecovery}\n/* ------------------------------------------------------------- session */\nasync function enterImmersive450(){`
);
replaceExact(
  'guard XR render frame',
  'function frame450(t,frame){const sess=S.sess;if(!sess)return;sess.requestAnimationFrame(frame450);',
  `function frame450(t,frame){\n if(!XRSTART450.firstFrameAt){\n  XRSTART450.firstFrameAt=xrNow450();\n  if(XRSTART450.firstFrameTimer){clearTimeout(XRSTART450.firstFrameTimer);XRSTART450.firstFrameTimer=0}\n  xrStage450('running','first XR frame');\n }\n try{return frame450Core(t,frame)}catch(e){\n  console.error('UPRS450 XR frame',e);\n  xrAbort450(S.sess,e,'render-frame');\n }\n}\nfunction frame450Core(t,frame){const sess=S.sess;if(!sess)return;sess.requestAnimationFrame(frame450);`
);
replaceExact(
  'direct trusted immersive-vr request',
  `async function enterImmersive450(){\n if(S.sess){try{await S.sess.end()}catch(_){}return}\n if(!navigator.xr){toast450(t450('noXR'));AU.setEnabled(true);const sel=currentSel450();if(sel)AU.apply(sel);return}\n let mode=null;\n try{if(await navigator.xr.isSessionSupported('immersive-ar'))mode='immersive-ar'}catch(_){}\n if(!mode){try{if(await navigator.xr.isSessionSupported('immersive-vr'))mode='immersive-vr'}catch(_){}}\n if(!mode){toast450(t450('noXR'));AU.setEnabled(true);const sel=currentSel450();if(sel)AU.apply(sel);return}\n const c=CORE();if(!c){try{return window.__UPRS440__?.enter()}catch(_){return}}\n try{\n  const opts={requiredFeatures:['local'],optionalFeatures:['local-floor','bounded-floor','hand-tracking','hit-test','anchors','depth-sensing'],\n   depthSensing:{usagePreference:['gpu-optimized','cpu-optimized'],dataFormatPreference:['luminance-alpha','float32']}};\n  const sess=await navigator.xr.requestSession(mode,opts);`,
  `async function enterImmersive450(){\n if(S.sess){try{await S.sess.end()}catch(_){}return}\n if(!navigator.xr){toast450(t450('noXR'));AU.setEnabled(true);const sel=currentSel450();if(sel)AU.apply(sel);return}\n const c=CORE();if(!c){toast450('XR core unavailable');return}\n const mode='immersive-vr';let sess=null,requestPromise=null;\n XRSTART450.startedAt=xrNow450();XRSTART450.sessionRequestedAt=0;XRSTART450.sessionAcquiredAt=0;\n XRSTART450.firstFrameAt=0;XRSTART450.refType='';XRSTART450.lastError=null;XRSTART450.history.length=0;\n try{\n  xrGuard450();\n  xrStage450('request-session','direct trusted click · immersive-vr');\n  XRSTART450.sessionRequestedAt=xrNow450();\n  requestPromise=navigator.xr.requestSession('immersive-vr',{optionalFeatures:['local-floor','bounded-floor']});\n  sess=await xrWait450(requestPromise,XRSTART450.policy.requestTimeoutMs,'request-session');\n  XRSTART450.sessionAcquiredAt=xrNow450();\n  xrStage450('session-acquired',sess.environmentBlendMode||'immersive-vr');`
);

const xrStart = 'async function enterImmersive450(){';
const xrEnd = '/* desktop sonification loop */';
replaceInRange(
  'non-blocking target frame rate', xrStart, xrEnd,
  '  try{if(sess.supportedFrameRates&&sess.updateTargetFrameRate){const rates=[...sess.supportedFrameRates];const want=rates.includes(90)?90:Math.max(...rates);await sess.updateTargetFrameRate(want);S.support.rate=want}}catch(_){}',
  '  try{if(sess.supportedFrameRates&&sess.updateTargetFrameRate){const rates=[...sess.supportedFrameRates].filter(Number.isFinite);if(rates.length){const want=rates.includes(90)?90:Math.max(...rates);Promise.resolve(sess.updateTargetFrameRate(want)).then(()=>{S.support.rate=want}).catch(()=>{})}}}catch(_){}'
);
replaceInRange(
  'finite WebGL compatibility stage', xrStart, xrEnd,
  "  S.gl=gl;await gl.makeXRCompatible();\n  const layer=new XRWebGLLayer(sess,gl,{alpha:true,antialias:true,framebufferScaleFactor:1});",
  "  S.gl=gl;xrStage450('gl-compatible',(typeof WebGL2RenderingContext!=='undefined'&&gl instanceof WebGL2RenderingContext)?'webgl2':'webgl1');\n  await xrWait450(gl.makeXRCompatible(),XRSTART450.policy.glTimeoutMs,'makeXRCompatible');\n  xrStage450('render-layer','XRWebGLLayer');\n  const layer=new XRWebGLLayer(sess,gl,{alpha:false,depth:true,stencil:false,antialias:true,framebufferScaleFactor:1});"
);
replaceExact(
  'finite reference-space fallback',
  "  S.ref=await sess.requestReferenceSpace('local-floor').catch(()=>sess.requestReferenceSpace('local'));\n  /* depth sensing */",
  "  xrStage450('reference-space','local-floor → local');\n  try{S.ref=await xrWait450(sess.requestReferenceSpace('local-floor'),XRSTART450.policy.referenceSpaceTimeoutMs,'local-floor');XRSTART450.refType='local-floor'}\n  catch(_){S.ref=await xrWait450(sess.requestReferenceSpace('local'),XRSTART450.policy.referenceSpaceTimeoutMs,'local');XRSTART450.refType='local'}\n  /* depth sensing is intentionally disabled in the default VR startup path; it is AR-only capability. */"
);
replaceInRange(
  'visible placement for local reference space', xrStart, xrEnd,
  "  S.world.pos=[0,mode==='immersive-ar'?1.05:1.15,-.75];",
  "  S.world.pos=[0,XRSTART450.refType==='local-floor'?1.15:-.08,-1.25];"
);
replaceExact(
  'first-frame watchdog',
  "  sess.requestAnimationFrame(frame450);\n  toast450(t450('started')+(S.support.rate?` · ${S.support.rate} Hz`:'')+(mode==='immersive-ar'?' · AR':' · VR'));",
  "  xrStage450('waiting-first-frame','render pipeline ready');\n  XRSTART450.firstFrameTimer=setTimeout(()=>{\n   if(!XRSTART450.firstFrameAt&&S.sess===sess){\n    const e=new Error('No XR animation frame');e.name='TimeoutError';\n    xrAbort450(sess,e,'first-frame');\n   }\n  },xrScale450(XRSTART450.policy.firstFrameTimeoutMs));\n  sess.requestAnimationFrame(frame450);\n  toast450('WebXR: запуск VR…');"
);
replaceExact(
  'session end cleanup',
  "  sess.addEventListener('end',()=>{S.sess=null;S.gl=null;S.sceneKey='';S.grabs.clear();btnPrev.clear();S.hands.clear();S.panel.tex=null;S.hit.source=null;S.anchorSpace=null;S.depth.binding=null;try{state.xrSession=null}catch(_){}toast450(t450('ended'))});",
  "  sess.addEventListener('end',()=>{if(XRSTART450.firstFrameTimer){clearTimeout(XRSTART450.firstFrameTimer);XRSTART450.firstFrameTimer=0}S.sess=null;S.gl=null;S.ref=null;S.sceneKey='';S.grabs.clear();btnPrev.clear();S.hands.clear();S.panel.tex=null;S.hit.source=null;S.anchorSpace=null;S.depth.binding=null;try{state.xrSession=null}catch(_){}if(XRSTART450.stage!=='failed')xrStage450('ended','session end');toast450(t450('ended'))},{once:true});"
);
replaceExact(
  'abort failed session without legacy retry',
  "}catch(e){console.warn('UPRS450 session',e);toast450('XR: '+e.message);try{window.__UPRS440__?.enter()}catch(_){}}",
  "}catch(e){console.warn('UPRS450 session',e);await xrAbort450(sess,e,XRSTART450.stage||'startup')}"
);
replaceExact(
  'Quest-safe button preflight',
  "  const btn=document.getElementById('xrBtn');if(btn)btn.onclick=()=>enterImmersive450();",
  "  const btn=document.getElementById('xrBtn');if(btn){btn.onclick=()=>enterImmersive450();btn.dataset.xrPolicy='quest-safe-v461';setTimeout(xrPreflight450,0)}"
);
replaceExact(
  'expose XR startup diagnostics',
  "globalThis.__UPRS450__={version:V450,enter:enterImmersive450,exit:()=>{try{S.sess?.end()}catch(_){}},toggleSound:toggleSound450,checks:checks450,",
  "globalThis.__UPRS450__={version:V450,enter:enterImmersive450,exit:()=>{try{S.sess?.end()}catch(_){}},toggleSound:toggleSound450,checks:checks450,startup:XRSTART450,startupPolicy:XRSTART450.policy,startupReport:xrReport450,preflight:xrPreflight450,"
);
...
scripts.forEach((code, index) => {
  try { new vm.Script(code, { filename: `inline-script-${index + 1}.js` }); }
  catch (error) { syntaxFailures.push({ index: index + 1, message: error.message }); }
});
if (syntaxFailures.length) throw new Error(`Inline script syntax failures: ${JSON.stringify(syntaxFailures, null, 2)}`);

const required = [
  'uprs460CoreAuthority','uprs460Consolidation','XRSTART450','quest-safe-v461',
  '__UPRS420__','__UPRS430__','__UPRS440__','__UPRS450__','__UPRS398__','asml_euv_lpp'
];
...
const report = {
  version: VERSION, source: SOURCE, sourceBytes, sourceSha256, outputBytes, outputSha256,
  inlineScripts: scripts.length, syntaxFailures, changes, requiredCapabilities: required,
  xrStartupPolicy: {
    mode: 'immersive-vr', directUserActivation: true, preferAR: false,
    depthOnStartup: false, autoLegacyFallback: false,
    requestTimeoutMs: 15000, glTimeoutMs: 8000, referenceSpaceTimeoutMs: 5000, firstFrameTimeoutMs: 8000
  }
};
